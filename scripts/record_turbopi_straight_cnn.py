"""Autonomous TurboPi straight-line recorder that writes legacy CNN episodes.

One successful episode = one traversal of the lane from the start end to the
finish end, with the robot finishing roughly on the centerline and facing
...
DEFAULT_OUTPUT_DIR = Path(__file__).resolve().parents[1] / "data" / "cnn_straight_line"

parser = argparse.ArgumentParser(description="Record autonomous TurboPi straight-line episodes for the no-text CNN pipeline.")
...
parser.add_argument("--dataset_name", type=str, default="turbopi_straight_line_cnn", help="Dataset name stored in session metadata.")
parser.add_argument("--num_episodes", type=int, default=8, help="Number of successful episodes to save.")
parser.add_argument(
    "--directions",
    type=str,
    default="forward",
    help="Comma-separated direction cycle, e.g. 'forward,reverse'.",
)
...
parser.add_argument("--episode_time_s", type=float, default=30.0, help="Hard timeout per rollout in seconds.")
parser.add_argument("--finish_tolerance", type=float, default=0.08, help="How close to the finish end in progress units before calling it done, in meters.")
parser.add_argument("--finish_yaw_tolerance", type=float, default=0.35, help="Required closeness to the finish heading when completing a run, in radians.")
parser.add_argument("--stuck_timeout_s", type=float, default=4.0, help="Abort if forward progress stalls for this long.")
parser.add_argument("--stuck_grace_period_s", type=float, default=3.0, help="Ignore stuck checks during startup.")
parser.add_argument("--max_track_error", type=float, default=0.25, help="Abort if the robot drifts farther than this from the centerline.")
...
parser.add_argument("--max_mean_track_error", type=float, default=0.06, help="Reject accepted runs whose mean track error is higher than this.")
parser.add_argument("--max_p90_track_error", type=float, default=0.10, help="Reject accepted runs whose p90 track error is higher than this.")
parser.add_argument("--max_peak_track_error", type=float, default=0.15, help="Reject accepted runs whose peak track error is higher than this.")
parser.add_argument("--max_frames_over_010_ratio", type=float, default=0.15, help="Reject accepted runs if more than this ratio of frames exceed 0.10 m track error.")
parser.add_argument("--min_image_std", type=float, default=8.0, help="Reject runs if the camera image stream is too flat or washed out.")
parser.add_argument("--max_mean_action_vy_vx_ratio", type=float, default=0.20, help="Reject runs if the mean absolute lateral action is too large relative to forward action.")
...
parser.add_argument("--target_speed", type=float, default=0.20, help="Nominal teacher speed in m/s.")
parser.add_argument("--heading_gain", type=float, default=2.2, help="Gain for yaw tracking. Picked so the discrete heading loop at 10 Hz with the corrected plant is overdamped instead of ringing.")
parser.add_argument("--cross_track_heading_gain", type=float, default=0.8, help="Extra yaw correction that turns the robot toward the centerline.")
parser.add_argument("--lateral_gain", type=float, default=0.35, help="Small residual lateral correction gain in body coordinates.")
parser.add_argument("--max_lateral_speed", type=float, default=0.045, help="Cap for lateral correction speed in m/s.")
parser.add_argument("--track_error_slowdown", type=float, default=0.35, help="Slowdown factor applied when the robot drifts off-center.")
...
parser.add_argument("--heading_slowdown_angle", type=float, default=0.70, help="Heading error angle beyond which forward speed is strongly reduced.")
...
def run_episode(
    *,
    ...
    observation = observe_line_state(robot, scene_cfg, direction)
    ...
    success = False
    last_print_at = -1.0
    best_progress = observation.progress
    last_progress_time = 0.0
    finish_progress = 2.0 * scene_cfg.line_half_length
    goal_yaw = 0.0 if direction == "forward" else math.pi
    ...
    if not camera_ready:
        ...
        return EpisodeResult(
            ...
            mean_abs_action_vx=0.0,
            mean_abs_action_vy=0.0,
            mean_abs_action_wz=0.0,
            mean_action_vy_vx_ratio=0.0,
            mean_speed=0.0,
            duration_s=0.0,
        )

    print(f"[INFO] Starting {direction} run.", flush=True)
    ...
    for step_index in range(max_steps):
        ...
        planar_speed = float(torch.linalg.norm(observation.body_velocity).item())
        track_errors.append(observation.track_error)
        body_speeds.append(planar_speed)
        if observation.progress > best_progress + 1e-4:
            best_progress = observation.progress
            last_progress_time = episode_time

        if observation.has_nan:
            print(
                f"[WARN] Invalid state: pos={observation.position_w.tolist()} "
                f"height={observation.height:0.4f} has_nan={observation.has_nan}",
                flush=True,
            )
        ...
        normalized_progress = max(0.0, min(1.0, observation.progress / max(finish_progress, 1e-6)))
        if (
            observation.progress >= finish_progress - args_cli.finish_tolerance
            and wrapped_angle_error(current_yaw, goal_yaw) <= args_cli.finish_yaw_tolerance
        ):
            terminal_reason = "run_complete"
            success = True
            break
        if observation.track_error > args_cli.max_track_error:
        ...
        frames.append(
            EpisodeFrame(
                ...
                body_velocity=torch.cat((observation.body_velocity, observation.body_ang_velocity)).detach().cpu().numpy(),
                track_error=observation.track_error,
                lap_progress=normalized_progress,
            )
        )
        ...
        if episode_time - last_print_at >= 1.0:
            ...
            print(
                f"[INFO] {direction:8s} t={episode_time:5.1f}s frames={len(frames):4d} "
                f"progress={observation.progress:0.2f}/{finish_progress:0.2f} "
                f"err={observation.track_error:0.3f} img_std={image_std:0.1f} "
                f"cmd=[{float(action[0]):0.2f}, {float(action[1]):0.2f}, {float(action[2]):0.2f}]",
                flush=True,
            )
    ...
    final_normalized_progress = max(0.0, min(1.0, best_progress / max(finish_progress, 1e-6)))
    ...
    if success and frames_over_010_ratio > args_cli.max_frames_over_010_ratio:
        success = False
        terminal_reason = "quality_frames_over_010_ratio"
    ...
    return EpisodeResult(
        ...
        final_lap_progress=final_normalized_progress,
...
def main() -> None:
    ...
    limits = StraightControlLimits(max_vx=args_cli.max_vx, max_vy=args_cli.max_vy, max_wz=args_cli.max_wz)
    teacher_cfg = StraightTeacherCfg(
    ...
    teacher = StraightLineTeacher(scene_cfg=scene_cfg, limits=limits, controller_cfg=teacher_cfg, device=robot.device)
    omega_tracker = None
    ...
    writer = CNNSessionWriter(
        ...
        physics_dt=args_cli.physics_dt,
        tasks=TASKS,
        track_layout="straight_tape_line",
        episode_definition="one_full_autonomous_traversal",
    )
    ...
    print()
    print("=" * 60)
    print("  TurboPi Autonomous Straight-Line Recorder")
    print("=" * 60)
    ...
    print(f"  Lane length      : {2.0 * scene_cfg.line_half_length:.2f} m")
    print(f"  Lane half width  : {scene_cfg.lane_half_width:.2f} m")
    print(f"  Max track error  : {args_cli.max_track_error:.2f} m")
    print(
        f"  Finish gate      : within {args_cli.finish_tolerance:.2f} m of end, "
        f"yaw<={args_cli.finish_yaw_tolerance:.2f} rad"
    )
    print(
        ...
        while simulation_app.is_running() and not stop_flag.requested and successful < args_cli.num_episodes and attempts < max_attempts:
            ...
            if result.success and result.frames:
                ...
                print(
                    ...
                    f"frames={len(result.frames)} progress={result.final_lap_progress:0.3f} "
                ...
                print(
                    ...
                    f"progress={result.final_lap_progress:0.3f} mean_err={result.mean_track_error:0.3f}",
                    flush=True,
                )
